...
#!/usr/bin/env pwsh
# .SYNOPSIS
#   cliHelper.semver testScript v0.1.0
# .EXAMPLE
#   ./Test-Module.ps1 -version 0.1.0
#   Will test the module in ./BuildOutput/cliHelper.semver/0.1.0/
# .EXAMPLE
#   ./Test-Module.ps1
#   Will test the latest  module version in ./BuildOutput/cliHelper.semver/
param (
    ...
    [ValidateScript({
            ...
            $b_Path = [IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'cliHelper.semver')
...
begin {
    $TestResults = $null
    $BuildOutput = [IO.DirectoryInfo]::New([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'cliHelper.semver'))
    ...
    $BuildOutDir = Resolve-Path $([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'cliHelper.semver', $version)) -ErrorAction Ignore | Get-Item -ErrorAction Ignore
    if (!$BuildOutDir.Exists) { throw [System.IO.DirectoryNotFoundException]::new($BuildOutDir) }
    $manifestFile = [IO.FileInfo]::New([IO.Path]::Combine($BuildOutDir.FullName, "cliHelper.semver.psd1"))
    ...
    if (!(Test-Path -Path $([IO.Path]::Combine($PSScriptRoot, "cliHelper.semver.psd1")) -PathType Leaf -ErrorAction Ignore)) { throw [System.IO.FileNotFoundException]::New("Module manifest file Was not Found in '$($BuildOutDir.FullName)'.") }
    $script:fnNames = [System.Collections.Generic.List[string]]::New(); $testFiles = [System.Collections.Generic.List[IO.FileInfo]]::New()
    [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'cliHelper.semver.Integration.Tests.ps1')))
    [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'cliHelper.semver.Features.Tests.ps1')))
    [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'cliHelper.semver.Module.Tests.ps1')))
}
